...
USAGE: rsync-snapshot.sh HOST [--recheck] [--noquest]
...
PURPOSE: create a snapshot backup of the whole filesystem into the folder
  ...
  backups with X>1024. About 10 backups with non-linear distribution are kept
  in rotation; for example with X=1,2,3,4,8,16,32,64,128,256,512,1024.
  ...
  The --noquest option means there will be no questions asked.
  Example: folders are not present or include files

FILES:
    /backup/snapshot/rsync/rsync-snapshot.sh  the backup script
    /backup/snapshot/rsync/rsync-list.sh      the md5 signature script
    /backup/snapshot/rsync/rsync-include_HOST.txt  the filter rules
...
HINTS:
  ...
  The following command can be very useful to replace all duplicate files with
  hard links to the first file in each set of duplicates, even if they have different name or path:

  chattr -fR -i /backup/snapshot/HOST/snapshot.*
  fdupes -r1L /backup/snapshot/HOST/snapshot.*

  Do you want to delete a snapshot, you must first delete the ‘chattr’ read-only protection by order:
  chattr -R -i snapshot.XXX

  After installing this script and its components set the ownerships and permissions:
  ...
  chmod 600 /backup/snapshot/rsync/rsync_hosts
...
OPT_NOQUEST="$3" #options (--noquest)
HOST_PORT=22 #port of source of backup
SCRIPT_PATH="/backup/snapshot/rsync"
SNAPSHOT_DST="/backup/snapshot/DATA" #destination folder
NAME="snapshot" #backup name
LOG="rsync.log"
ERR_MESS=0 # when checking the parameters in failure case count up
NOQUEST=0 # dont ask questions parameter in case flder or include file don't exist
...
MAX_MIBSIZE=1000000 # older snapshots (except snapshot.001) are removed if their size >= MAX_MIBSIZE. the script performs a backup even if their size is too big.
...
BWLIMIT=100000 # bandwidth limit in KiB/s. 0 does not use slow-down. this allows to avoid rsync consuming too much system performance - old BWLIMIT=100000
...
if [ -z "${SRC}" ] || [ "${SRC}" == "localhost" ]; then
  ...
  if [ "${SRC}" == "--recheck" ] || [ "${SRC}" == "--noquest" ]; then
    echo "Syntax error !"
    echo "Try rsync-snapshot.sh --help .Exiting..."
    exit 2
  fi
...
if [ "${HOST_SRC}" != "${HOST_LOCAL}" ]; then #option for a remote server
  ...
  if  [ ! -f /root/.ssh/rsync_rsa_${SRC} ];then  #look for rsa keys
	echo "Sorry, please create rsa keys first"
	echo "Example: "
	echo "\"ssh-keygen -f ~/.ssh/rsync_rsa_${SRC}\"  <-- create Key "
	echo "\"MYIP=\$(hostname -i) #assign here the backupserver's external IP if necessary\""
	echo "\"echo \"from="\${MYIP%% *}",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty,command=\"rsync \${SSH_ORIGINAL_COMMAND#* }\" \$(ssh-keygen -yf ~/.ssh/rsync_rsa_${SRC})\" | ssh ${SRC} \"cat - >>~/.ssh/authorized_keys\""
        echo "or:"
        echo "\"ssh-copy-id -i ~/.ssh/rsync_rsa_${SRC}.pub root@${SRC}\"  <-- Copy Key to ssh server "
	echo "Exiting..."
  ...
  OPTION="${OPTION} \
  --compress \
  --rsh=\"ssh -p ${HOST_PORT} -i /root/.ssh/rsync_rsa_${SRC} -l root\" \
  --rsync-path=\"/usr/bin/rsync\""
fi

 # Check for parameters
if [ "${OPT}" == "--recheck" ] || [ "${OPT_NOQUEST}" == "--recheck" ] ; then
  OPTION="${OPTION} \
  --ignore-times"
fi
if [ "${OPT}" == "--noquest" ] || [ "${OPT_NOQUEST}" == "--noquest" ] ; then
  NOQUEST=1
fi
if [ -n "${OPT}" ] && [ ! "${OPT}" == "--recheck" ] && [ ! "${OPT}" == "--noquest" ]; then
  ERR_MESS=1
fi
if [ -n "${OPT_NOQUEST}" ] && [ ! "${OPT_NOQUEST}" == "--recheck" ] && [ ! "${OPT_NOQUEST}" == "--noquest" ]; then
  ERR_MESS=$(($ERR_MESS+1))
fi
if [ $ERR_MESS -gt 0 ];then
  echo "Bad Parameter. Try rsync-snapshot.sh --help ."
  exit 2
fi

# point to stop

#read -n 1 -p "Continue? [y/n] " QUEST
#if [ ! "${QUEST}" == "y" ];then
#  echo -e "\n"
#  exit 2
#fi
...
if [ ! -d "${SNAPSHOT_DST}/${HOST_SRC}" ]; then
  echo "Folder ${SNAPSHOT_DST}/${HOST_SRC} is missing."
  if [ ! -d "${SNAPSHOT_DST}" ]; then
    echo "Folder ${SNAPSHOT_DST} is missing. Need configaration. Exiting..."
    echo "$(date +%Y-%m-%d_%H:%M:%S) ${HOST_SRC}: === Snapshot failed. ==="
    exit 2
  fi
  mkdir ${SNAPSHOT_DST}/${HOST_SRC}
  if [ ! -d "${SNAPSHOT_DST}/${HOST_SRC}" ]; then
    echo "Error: Failure to create folder ${SNAPSHOT_DST}/${HOST_SRC} ! Exiting..."
    echo "$(date +%Y-%m-%d_%H:%M:%S) ${HOST_SRC}: === Snapshot failed. ==="
    exit 2
  fi
  echo "Folder ${SNAPSHOT_DST}/${HOST_SRC} created."
fi
...
if [ "${HOST_LOCAL}" != "${BACKUPSERVER}" ]; then #because BACKUPSERVER need sometimes to perform an rsync-cp.sh it must disable the check of "already started".
  ...
  if pgrep -f "\w*sh \w*rsync-snapshot\.sh" | grep -qv "$$"; then
...
if [ ! -f "${SCRIPT_PATH}/rsync-include_${SRC}.txt" ];then
   echo "file ${SCRIPT_PATH}/rsync-include_${SRC}.txt not exist."
   if [ ! "${NOQUEST}" -eq 1 ];then # continue, when Parameter --noquest is not given
     read -n 1 -p "Creating a copy from original and edit them? [y/n] " QUEST
     if [ "${QUEST}" == "y" ];then
       echo -e "\n"
       cp "${SCRIPT_PATH}/rsync-include-original.txt" "${SCRIPT_PATH}/rsync-include_${SRC}.txt"
       nano "${SCRIPT_PATH}/rsync-include_${SRC}.txt"
       read -n 1 -p "Do you want to continue rsync with edited rsync-include_${SRC}.txt? [y/n] " QUEST2
       if [ ! "${QUEST2}" == "y" ];then
         echo -e "\ņExiting..."
         echo "$(date +%Y-%m-%d_%H:%M:%S) ${HOST_SRC}: === Snapshot failed. ==="
	 exit 2
...
remove_snapshot() {
  ...
  for i in $(seq -f'%03g' 1024 -1 001); do
...
while :; do #this loop is executed a 2nd time if OVERWRITE_LAST was ==1 and snapshot.001 got removed
  ...
  eval rsync \
    --dry-run \
    ${OPTION} \
    --include-from="${SCRIPT_PATH}/rsync-include_${SRC}.txt" \
    ${LNKDST} \
...
# rotate the backups
if [ -d "${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.1024" ]; then #remove snapshot.1024
  echo "Removing ${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.1024 ..."
  [ "${CHATTR}" -eq 1 ] && chattr -R -i "${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.1024" &>/dev/null
  rm -rf "${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.1024"
fi
[ -h "${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.last" ] && rm -f "${SNAPSHOT_DST}/${HOST_SRC}/${NAME}.last"
